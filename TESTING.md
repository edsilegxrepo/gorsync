...
| **Authentication** | `TestE2EAuthDaemon` | Validates MD4 challenge-response authentication with `auth_users` & `secrets_file`. | **PASS**: Unauthenticated request rejected; `--password-file` and `RSYNC_PASSWORD` succeed. |
